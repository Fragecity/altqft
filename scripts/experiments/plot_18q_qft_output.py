#!/usr/bin/env python3
"""Sample and plot two 18-qubit QFT output distributions.
...
DEFAULT_OUTPUT_DIR = Path("outputs/18q_qft_p3_q5")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the 18q QFT on |V_p> and |V_q>, sample both output distributions "
            "in parallel, and plot P(x)."
        )
    )
...
def _simulate_and_sample(task: SimulationTask) -> SimulationResult:
    ...
    with torch.inference_mode():
        ...
        probabilities_exact = probabilities.detach().cpu().to(torch.float64).numpy()
