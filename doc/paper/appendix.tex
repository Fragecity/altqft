...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.9\linewidth]{../../figs/vpq_dist.pdf}
...
\(b_i\in\{0,1\}^{n}\), and let \(P(b\mid r)\) be the output probability of the
HP circuit for a candidate period \(r\).  The MLE is
\begin{equation}
    \widehat r_{\mathrm{ML}}
    =
    \underset{r\in\mathcal R}{\operatorname{arg\,max}}\;
    \prod_{i=1}^{m}P(b_i\mid r)
    =
    \underset{r\in\mathcal R}{\operatorname{arg\,max}}\;
    \sum_{i=1}^{m}\log P(b_i\mid r),
...
between the full distributions \(P(\cdot\mid r)\) for different \(r\); therefore the DFI alone should not be interpreted as a proof of an achievable decoding error exponent.  The empirical MLE failure rates shown in
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.9\linewidth]{../../figs/mle.pdf}
...
Eq.~\eqref{eq:decoder_mle} requires computing \(P(b_i\mid r)\) for every observed bit string and every candidate period.  A direct construction of the complete output distribution contains \(2^n\) probabilities and requires repeated circuit simulation, which is prohibitive in the regime of interest.  A specialized likelihood evaluation with lower cost is possible in principle, but its implementation and complexity are not established here.  We therefore replace the explicit likelihood calculation by a learned estimator that acts directly on the sampled bit strings.
...
Eq.~\eqref{eq:decoder_residual_block} act on \(z(B_m)\).  A final linear layer
produces one logit for each candidate period.  In the reported model
\(n=18\), so \(d_e=18\), \(nd_e=324\), \(d=288\), and the hidden dimension in
each residual block is \(2d=576\).  The output classes are the \(510\) periods
\(\mathcal R=\{2,\ldots,511\}\), and the complete network has \(1{,}909{,}230\)
trainable parameters.

The implemented scoring head is a multiclass classifier rather than an
autoregressive sequence model.  Applying log-softmax to its \(510\) logits and
sorting them gives the exact top-\(k\) candidates; the experiments retain
\(k=10\) for subsequent classical verification.  Thus, the beam label in the
schematic architecture refers only to retaining a short ranked list: no
stepwise beam expansion or separately chosen beam width is used.  At inference,
the protocol supplies \(m=1024n^2\) circuit measurements (\(331{,}776\) for
\(n=18\)); count compression and Eq.~\eqref{eq:decoder_deep_sets} make the
network input depend on the number of distinct outcomes rather than on storing
all \(m\) strings separately.

For training, cached HP-1 output distributions were constructed for every
\(r\in\{2,\ldots,511\}\) and every subgroup shift \(s\in\{0,\ldots,r-1\}\).
One shift per period was reserved before training, leaving \(130{,}305\)
period--shift rows available for training.  Each data item was generated by
drawing \(8192\) outcomes from one cached distribution and compressing them to
distinct bit strings with empirical-frequency weights.  At each epoch, \(96\)
periods were selected and \(1024\) training items were drawn; the mini-batch
size was \(8\).  Validation used a fixed subset of \(512\) draws from the
held-out shifts.

The network was trained with AdamW using learning rate \(10^{-3}\), weight
decay \(10^{-4}\), the default momentum parameters
\((\beta_1,\beta_2)=(0.9,0.999)\), and cross-entropy loss with label smoothing
\(0.02\).  All residual-block dropout probabilities were \(0.1\).  The random
seed was \(7\), and CUDA training used bfloat16 autocasting.  The \(510\)-class
run was initialized from a checkpoint trained on \(322\) candidate periods:
all shape-compatible shared tensors and the classifier rows corresponding to
overlapping periods were copied.  Checkpoints were selected by validation
top-1 accuracy, with a minimum of \(300\) epochs and early-stopping patience
\(250\).  Training stopped after epoch \(1919\); the selected checkpoint was
epoch \(1669\), with validation loss \(0.4845\), top-1 accuracy \(0.9551\), and
top-10 accuracy \(1.0000\).

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.6\linewidth]{../../figs/ml/ml_arch.pdf}
    \caption{\textbf{Neural decoder architecture.} The HP circuit produces an
    unordered set of measurement bit strings.  A shared feature map processes
    every distinct bit string, the resulting features are aggregated by a
    count-weighted mean, and a multiclass scoring head produces a ranked list of
    candidate periods.}
    \label{fig:decoder_architecture}
\end{figure}

The use of a neural decoder should be distinguished from an analytical decoding theorem.  The DFI characterizes the sensitivity of the HP output distribution to the hidden period and is therefore used here as an information-theoretic resolution scale~\cite{weimar2025fisher}.  Once the likelihood is replaced by a trained neural estimator, the DFI is not an asymptotically achieved bound for that estimator; the actual decoder performance must be established empirically through validation accuracy, top-\(k\) recovery, and the factorization tests.  In this sense, the neural network demonstrates that the information diagnosed by the DFI is practically accessible in the tested regime, while a general polynomial-cost
decoding guarantee remains an open question.
