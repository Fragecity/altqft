...
\(C\)-twisted dyadic-resonance estimate gives, at the square-root period
window \(r=2^{n/2}=\sqrt N\),
\[
    S_r
    :=
...
\begin{proof}
For the \(C\)-twisted formulation, write \(b(m)\in\{0,1\}^n\) for the \(n\)-bit
register string of the integer \(m\), and set
\begin{equation}
    v_{s,t}(p,q;p',q')
    :=
...
Lemma~\ref{lem:hp1_measurement_probability_expansion} gives the coherent
period-\(s\) law in Eq.~\eqref{eq:hp1_lag_correlation_probability}.  We use
that conclusion in the form
\[
    \Pr_s(x)
    =
    \frac{1}{N R^{(s)}}
    \left|
    \sum_{q=0}^{R^{(s)}-1}
    \prod_{j=1}^{n}w_j(x)^{(qs)_j}
    \right|^2 .
\]
Applying this formula to \(s=r\) and \(s=r+1\), the probability difference can
be expanded directly.  In the expansions below,
integer products such as \(ps\) are read as \(n\)-bit register strings.  Then
\begin{equation}
    \begin{aligned}
    \Delta_r\Pr(x)
    =
    ...
    &\frac{1}{R^{(r+1)}}
    \sum_{p,q=0}^{R^{(r+1)}-1}
    \prod_{j=1}^{n}w_j(x)^{(p(r+1))_j}
    \overline{\prod_{j=1}^{n}w_j(x)^{(q(r+1))_j}} \\
    &-
    \frac{1}{R^{(r)}}
    \sum_{p,q=0}^{R^{(r)}-1}
    \prod_{j=1}^{n}w_j(x)^{(pr)_j}
    \overline{\prod_{j=1}^{n}w_j(x)^{(qr)_j}}
    \biggr].
    \end{aligned}
    \label{eq:hp1_probability_difference_coherent_expansion}
\end{equation}
For a pair of periods \(s,t\), the overlap calculation only needs the inner
\(x\)-sum over a fixed quadruple of modes.  Using the bitwise phase form in
Eq.~\eqref{eq:hp1_bitwise_phase_factor}, this sum factorizes as
\begin{equation}
\begin{aligned}
    \mathcal K_{s,t}(p,q;p',q')
    &:=
    \sum_{x\in\{0,1\}^n}
    \prod_{j=1}^{n}w_j(x)^{(ps)_j}
    \overline{\prod_{j=1}^{n}w_j(x)^{(qs)_j}}
    \prod_{j=1}^{n}w_j(x)^{(p't)_j}
    \overline{\prod_{j=1}^{n}w_j(x)^{(q't)_j}} \\
    &=
    \prod_{i=1}^{n}
    \left[
        1+
...
\end{equation}
Thus
\(\mathcal K_{s,t}(p,q;p',q')=N\Gamma_C(v_{s,t}(p,q;p',q'))\), so each
overlap \(\sum_x\Pr_s(x)\Pr_t(x)\) equals \(N^{-1}\mathcal E_C(s,t)\).
Applying this to the adjacent numerator gives the exact identity
\begin{equation}
    \sum_{x\in\{0,1\}^n}\bigl[\Delta_r\Pr(x)\bigr]^2
    =
    \frac1N
...
The kernel \(\Gamma_C\) gives a soft \(C\)-resonance condition.  Since
...
We now use this identity as a scaling diagnostic for the adjacent numerator,
not as a theorem-level lower bound.  Set
\begin{equation}
    S_r:=
    \sum_{x\in\{0,1\}^n}
    \bigl(\Pr_{r+1}(x)-\Pr_r(x)\bigr)^2 .
    \label{eq:hp1_c_twisted_adjacent_numerator_definition}
\end{equation}
and
\begin{equation}
    I_{s,t}:=\sum_x \Pr_s(x)\Pr_t(x).
    \label{eq:hp1_c_twisted_overlap_definition}
\end{equation}
Then
\begin{equation}
    S_r=I_{r+1,r+1}+I_{r,r}-2\operatorname{Re}I_{r+1,r}.
    \label{eq:hp1_c_twisted_overlap_decomposition}
\end{equation}
The exchanged-sum calculation above gives
\begin{equation}
    I_{s,t}=\frac1N\mathcal E_C(s,t),
    \label{eq:hp1_c_twisted_overlap_energy_identity}
\end{equation}
with \(\mathcal E_C\) defined in Eq.~\eqref{eq:hp1_c_twisted_energy} and
\(\Gamma_C\) defined in Eq.~\eqref{eq:hp1_c_twisted_kernel}.  Therefore
\begin{equation}
    S_r
    =
    \frac1N
    ...
    2\operatorname{Re}\mathcal E_C(r+1,r)
    \right].
    \label{eq:hp1_c_twisted_scaling_identity}
\end{equation}

The remaining step is statistical rather than algebraic.  The fixed-phase
\(\myqc-1\) matrix does not produce a generic random phase hash.  Because its
off-diagonal entries are dyadic, periods with large \(2\)-adic valuation
produce many near-resonant modes.  Write
\begin{equation}
    s=2^{a}u,\qquad u\ \text{odd},\qquad a=\nu_2(s).
    \label{eq:hp1_period_2adic_decomposition}
\end{equation}
Here \(\nu_2(m)\) denotes the \(2\)-adic valuation of \(m\), namely the largest
integer \(a\) such that \(2^a\) divides \(m\).
For example, if \(s=40\), then
\begin{equation}
    40=2^3\cdot 5,
    \label{eq:hp1_2adic_decomposition_example}
\end{equation}
so \(a=\nu_2(40)=3\) and \(u=5\).
Then all multiples \(qs\) have \(a\) fixed trailing zero bits.  These fixed bits
pass through the exact parity filters on \(\Lambda_2\) and also suppress part
of the dyadic phase hash on \(\Lambda_1\).  Thus the number of \(C\)-resonant
modes is enhanced by a factor of order \(2^a\).  The remaining finite-register
correction is treated as an order-one residual.  The statistical input used
below is the corresponding cross-period version of this observation: two periods
can share only the trailing-zero structure common to both of them.  We summarize
this by the scaling heuristic
\begin{equation}
    \mathcal E_C(s,t)-1
    =
    \frac{N}{st}\,
...
\end{equation}
for cross overlaps.  The factor \(2^{\min(\nu_2(s),\nu_2(t))}\) records the
common dyadic depth, while the \(\Theta(1)\) factor absorbs finite-register
boundary effects and the soft width of the near-resonance condition
\(|\Gamma_C(v)|\approx1\).  This bounded-residual assumption is checked
numerically in Fig.~\ref{fig:hp1_c_twisted_dyadic_resonance}: the right panel
fixes the dyadic depth and shows no visible polynomial or exponential drift in
\(n\) after the \(N2^a/(st)\) scale has been divided out.
...
\begin{figure}[h]
    ...
    Numerical check of the dyadic-resonance scaling in
    Eq.~\eqref{eq:hp1_c_twisted_cross_energy_heuristic}.  The data are obtained
    by exact state-vector evaluation at \(n=26\) for the fixed-phase
    \(\myqc-1\) kernel, using \(s=2^a u\), \(t=2^a v\) over \(30\) odd
    pairs \((u,v)\) and \(a=0,\ldots,14\).  The left panel plots
...
adjacent difference in Eq.~\eqref{eq:hp1_c_twisted_scaling_identity}.
...
\end{equation}
Substituting Eq.~\eqref{eq:hp1_c_twisted_dyadic_energy_heuristic} into
Eq.~\eqref{eq:hp1_c_twisted_scaling_identity} gives
\begin{equation}
    S_r
    =
    \Theta(1)
...
At the upper edge of the square-root period window, take
\begin{equation}
    r=2^{n/2}=\sqrt N.
...
\end{equation}
This is the claimed square-root-window numerator scaling.
\end{proof}
...
For \myqc-1, the minimum adjacent-period DFI obeys the ln-linear scaling
\begin{equation}
...
\begin{proof}
The discrete Fisher information is
\begin{equation}
    \fii(r,n)
    =
...
\end{equation}
If the dominant contribution comes from outputs where
\begin{equation}
...
\begin{equation}
    \fii(r,n)
    \approx
    N S_r\,\operatorname{poly}(n)^{-1}.
    \label{eq:hp1_dfi_from_numerator_heuristic}
\end{equation}
Combining this with
Eq.~\eqref{eq:hp1_c_twisted_square_root_window_numerator} gives
\begin{equation}
    \fii(r,n)
    \sim
...
\end{equation}
Thus the DFI is ln-linear up to sublinear corrections on the square-root period
windows.  The finite-window exponent and intercept are then determined by the
least-squares fit to the numerical \(\fii_{\min}(n)\) data, yielding
