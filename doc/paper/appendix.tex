...
Since the function $f$ is constant on the cosets of the hidden subgroup $V$ and distinct on different cosets, it is highly advantageous to group the terms in the superposition according to these cosets.
...
\begin{align}
    \text{QFT} \ket{c+V} &= \frac{1}{\sqrt{|V|}} \sum_{v \in V} \text{QFT} \ket{c+v} \nonumber \\
...
\begin{equation}
    T_g\ket{h}=\ket{h+g}.
\end{equation}
For a subgroup $V\le G$, write the normalized subgroup state as
\begin{equation}
...
\begin{equation}
    ...
    =
    \left|\bra{x}U T_g\ket{V}\right|^2,
    \qquad
    \forall x,g\in G,\ \forall V\le G.
    \label{eq:prob_shift_invariance}
\end{equation}
Thus $U\ket{V}$ and $U\ket{g+V}$ may differ by phases, but they must produce the same probabilities in the computational basis.
...
\begin{proof}[Proof of Theorem~\ref{thm:necessary_modulus_condition}]
    ...
    Then $\ket{V}=\ket{0}$ and $T_g\ket{V}=\ket{g}$ for every $g\in G$.
...
\begin{proof}
    ...
    \begin{align}
        &\cpgate{\Lambda_1}{\bar{\Lambda}_1}{\bm{\theta}_{\Lambda_1\bar{\Lambda}_1}} \bigotimes_{i \in \Lambda_1} H_{i}\ket{b_1 b_2\cdots b_n}\notag\\
        =& \cpgate{\Lambda_1}{\bar{\Lambda}_1}{\bm{\theta}_{\Lambda_1\bar{\Lambda}_1}} \bigotimes_{i \in \Lambda_1}\frac{1}{\sqrt{2}}(\ket{0} + e^{\pi \ii b_i} \ket{1}) \bigotimes_{k \in \bar \Lambda_1} \ket{b_k} \\
        =& \bigotimes_{i \in \Lambda_1}\frac{1}{\sqrt{2}}\left(\ket{0} + \exp\left[\pi \ii b_i + \ii \sum_{j \in \bar{\Lambda}_1} \theta_{ij} b_j\right] \ket{1}\right) \bigotimes_{k \in \bar \Lambda_1} \ket{b_k}.
    \end{align}
    Note that since the qubits in $\bar{\Lambda}_1$ have not yet been acted upon by Hadamard gates, they remain in the computational basis. 
    
    Since the control subsets form a partition of the qubits:
    \begin{equation}
        \bigcup_{l=1}^L \Lambda_l = \{1,2,\cdots, n\}, \quad
        \Lambda_i \cap \Lambda_j = \emptyset,
    \end{equation}
    and the target subsets are defined as the remaining unused qubits:
    \begin{equation}
        \bar{\Lambda}_l = \{1,2,\cdots, n\}\setminus \left(\bigcup_{k = 1}^l \Lambda_k\right),
    \end{equation}
    the sequential action of all $L$ layers ensures every qubit $i$ acts as a target exactly once. Let $l(i)$ denote the specific stage where $i \in \Lambda_{l(i)}$. The final state after the full circuit becomes:
    \begin{equation}
    ...
    \begin{align}
        ...
        =& \frac{1}{\sqrt{2^n}} \exp\left[ \ii \pi \sum_{i=1}^n b_i b'_i + \ii \sum_{l=1}^{L-1} \sum_{i \in \Lambda_l} \sum_{j \in \bar{\Lambda}_l} \theta_{ij} b'_i b_j \right].
    \end{align}
