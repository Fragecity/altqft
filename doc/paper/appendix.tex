...
For the fixed-phase \myqc-1 circuit, let \(\Pr_s\) and \(\Pr_t\) denote the
coherent output probability laws associated with two candidate periods \(s,t\).
Their overlap is
\begin{equation}
    \sum_x \Pr_s(x)\Pr_t(x)
    =
...
expression for \(\Pr_t\) into Eq.~\eqref{eq:hp1_period_law_overlap_placeholder}
gives
	\begin{align}
	    \sum_x \Pr_s(x)\Pr_t(x)
	    &=
...
to the probability overlap \(\sum_x\Pr_s(x)\Pr_t(x)\).

\begin{figure}[t]
    \includegraphics[width=\columnwidth]{hp1_cos_zero_vs_alpha_n500_alpha0_100_500per_density.pdf}
    \caption{\label{fig:hp1_zero_cosine_alpha_density}
    Gaussian-splat density plot of the empirical probability that
    \(\cos[\pi(C\Delta_{q,q',0,0})_a/2]=0\), grouped by
    \(\alpha=\nu_2((q'-q)s)\).  The data use \(n=500\) and 500 stratified
    samples for each \(\alpha=0,\ldots,100\).  The dashed curve is the
    empirical mean model, and the dotted curves show its \(\pm2\sigma_\alpha\)
    band.
    }
\end{figure}
...
\begin{equation}
    ...
    \left\{
        ...
        \cos\!\left[
            \frac{\pi}{2}
            \bigl(C\Delta_{q,q',0,0}\bigr)_a
        \right]=0
    \right\}
    \right|.
    \label{eq:hp1_zero_factor_frequency}
\end{equation}
The empirical distribution of \(P_\alpha\) is well described by
\begin{equation}
    \begin{aligned}
    P_\alpha(q,q',s)
    &\approx
    \mathcal N\!\left(\mu_\alpha,\sigma_\alpha^2\right),\\
    \mu_\alpha
    &=
    \frac14-
    \frac{\lfloor\alpha/2\rfloor-\mathbf 1_{\alpha\ \mathrm{odd}}}{2n},
    \qquad
    \sigma_\alpha^2
    =
    \frac{n/2-\lceil\alpha/2\rceil}{4n^2}.
    \end{aligned}
    \label{eq:hp1_zero_factor_gaussian_closure}
\end{equation}
The reason for this form is the staggered odd/even structure of the HP-1 matrix
\(C\).  Conditioning on \(\alpha\) freezes a trailing-zero depth in
\(\Delta_{q,q',0,0}\).  Since the dyadic part of \(C\) couples odd rows to even
columns, every two additional trailing zeros remove roughly one active dyadic
hash constraint, giving the drift in \(\mu_\alpha\); the odd-\(\alpha\)
correction comes from the row-parity staggering.  The remaining
\(n/2-\lceil\alpha/2\rceil\) active contributions behave like weakly dependent
Bernoulli fluctuations, so averaging over \(n\) rows gives
\(\operatorname{Var}(P_\alpha)\approx
(n/2-\lceil\alpha/2\rceil)/(4n^2)\).
\end{proof}
