...
\begin{equation}
    ...
    =
    \prod_{j=1}^{n}
    \exp\left(\frac{\ii}{2}\varphi_j(x)\right)
    \left[\cos\left(\frac{\varphi_j(x)}{2}\right)\right]^{1-\Delta_j}
    \left[-\ii\sin\left(\frac{\varphi_j(x)}{2}\right)\right]^{\Delta_j}.
\end{equation}
...
\begin{proof}
    ...
    Therefore
    \begin{equation}
        \exp(\ii y_j\varphi_j(x))
        =
        \exp\left(\frac{\ii}{2}\varphi_j(x)\right)
        \left[
            \cos\left(\frac{\varphi_j(x)}{2}\right)
            -\ii(-1)^{y_j}\sin\left(\frac{\varphi_j(x)}{2}\right)
        \right].
    \end{equation}
    Multiplying over $j$ and collecting terms by the mask $\Delta$ gives
    \begin{equation}
        \exp\left(\ii\sum_{j=1}^{n}y_j\varphi_j(x)\right)
        =
        \sum_{\Delta\in\{0,1\}^n}
...
arbitrary circuit. We therefore separate the general Fisher-information
stability statement from the fixed-phase \myqc-1 attenuation formula.

For any probability family $\Pr(x\mid r)$, write
\begin{equation}
...
\begin{equation}
    ...
    :=
    \sum_{x=0}^{2^n-1}
    \frac{\bigl(\Delta_r\Pr(x\mid r)\bigr)^2}{\Pr(x\mid r)}.
\end{equation}
The denominator is always evaluated on the support where the corresponding term
is positive. To handle coherent components that vanish at some outputs, we use
the following convention. Given a nonnegative component $s_n(x\mid r)$ and a
signed remainder $b_n(x\mid r)$, set $p_n=s_n+b_n$. We say that $b_n$ is
Fisher-small relative to $s_n$ if there exist sets $S_{n,r}$ such that
$s_n(x\mid r)>0$ on $S_{n,r}$,
\begin{equation}
    \varepsilon_n
    :=
    \max\left\{
    \left\|\frac{b_n}{s_n}\right\|_{\infty,S_{n,r}},
    \frac{\|\Delta_r b_n\|_{s_n,S_{n,r}}}
    {\|\Delta_r s_n\|_{s_n,S_{n,r}}}
    \right\}
    \longrightarrow 0,
\end{equation}
where
\begin{equation}
    \|f\|_{s_n,S_{n,r}}^2
    :=
    \sum_{x\in S_{n,r}}\frac{f(x)^2}{s_n(x\mid r)},
\end{equation}
and the contribution of $S_{n,r}^{c}$ to the relevant Fisher sums satisfies
\begin{equation}
    \sum_{\substack{x\notin S_{n,r}\\ p_n(x\mid r)>0}}
    \frac{\bigl(\Delta_r p_n(x\mid r)\bigr)^2}{p_n(x\mid r)}
    +
    \sum_{\substack{x\notin S_{n,r}\\ s_n(x\mid r)>0}}
    \frac{\bigl(\Delta_r s_n(x\mid r)\bigr)^2}{s_n(x\mid r)}
    =
    o\left(
    \sum_{x\in S_{n,r}}
    \frac{\bigl(\Delta_r s_n(x\mid r)\bigr)^2}{s_n(x\mid r)}
...
\begin{lemma}[DFI transfer under a Fisher-small remainder]
\label{lem:dfi_stability_remainder}
    Let
    \begin{equation}
        \Pr(x\mid r)
        =
        a_n(x)\Pr\nolimits_{\mathrm{ref}}(x\mid r)
        +
        \Pr\nolimits_{\mathrm{bg}}(x\mid r),
    \end{equation}
    where $a_n(x)\geq0$ is independent of $r$, and
    $\Pr\nolimits_{\mathrm{bg}}$ is a signed remainder. If
    $\Pr\nolimits_{\mathrm{bg}}$ is Fisher-small relative to
    $a_n\Pr\nolimits_{\mathrm{ref}}$, then
    \begin{equation}
        \fii_{\Pr}(r,n)
        =
        \bigl(1+o(1)\bigr)A_n(r)\fii_{\mathrm{ref}}(r,n),
    \end{equation}
    where
    \begin{align}
        \fii_{\mathrm{ref}}(r,n)
        &:=
        \fii_{\Pr_{\mathrm{ref}}}(r,n),\\
        A_n(r)
        &:=
        \frac{
        \displaystyle
        \sum_x a_n(x)
        \frac{
        \bigl(\Delta_r\Pr\nolimits_{\mathrm{ref}}(x\mid r)\bigr)^2
        }{
        \Pr\nolimits_{\mathrm{ref}}(x\mid r)
        }
        }{
    ...
    Consequently, if
    \begin{equation}
        \fii_{\mathrm{ref}}(r,n)=\exp[k_{\mathrm{ref}}n+o(n)],
        \qquad
        A_n(r)=\exp[-\gamma n+o(n)],
    \end{equation}
    then
    \begin{equation}
        \fii_{\Pr}(r,n)=\exp[(k_{\mathrm{ref}}-\gamma)n+o(n)].
    \end{equation}
\end{lemma}
...
\begin{proof}
    Set
    \begin{equation}
        \Pr_0(x\mid r):=a_n(x)\Pr\nolimits_{\mathrm{ref}}(x\mid r).
    \end{equation}
    On $S_{n,r}$, Fisher-smallness gives
    \begin{equation}
        (1-\varepsilon_n)\Pr_0(x\mid r)
        \leq
        \Pr(x\mid r)
        \leq
        (1+\varepsilon_n)\Pr_0(x\mid r),
    ...
    and
    \begin{equation}
        (1-\varepsilon_n)\|\Delta_r\Pr_0\|_{\Pr_0,S_{n,r}}
        \leq
        \|\Delta_r\Pr\|_{\Pr_0,S_{n,r}}
        \leq
        (1+\varepsilon_n)\|\Delta_r\Pr_0\|_{\Pr_0,S_{n,r}}.
    \end{equation}
    Therefore the Fisher sums on $S_{n,r}$ obey
    \begin{equation}
        \frac{(1-\varepsilon_n)^2}{1+\varepsilon_n}\fii_0(S_{n,r})
        \leq
        \fii_{\Pr}(S_{n,r})
        \leq
        \frac{(1+\varepsilon_n)^2}{1-\varepsilon_n}\fii_0(S_{n,r}),
    \end{equation}
    where $\fii_{\Pr}(S_{n,r})$ and $\fii_0(S_{n,r})$ denote the corresponding
    Fisher sums restricted to $S_{n,r}$, and
    \begin{equation}
        \fii_0(S_{n,r})
        :=
        \sum_{x\in S_{n,r}}
        \frac{\bigl(\Delta_r\Pr_0(x\mid r)\bigr)^2}{\Pr_0(x\mid r)}.
    \end{equation}
    The negligible-tail assumption extends this equivalence to the full Fisher
    sums. Since $a_n$ is independent of $r$,
    \begin{equation}
        \sum_x
        \frac{\bigl(\Delta_r\Pr_0(x\mid r)\bigr)^2}{\Pr_0(x\mid r)}
        =
        ...
        =
        A_n(r)\fii_{\mathrm{ref}}(r,n).
    \end{equation}
    This proves the transfer formula, and the exponent statement follows by
    substitution.
\end{proof}

\begin{theorem}[Fixed-phase \myqc-1 attenuation and conditional DFI scaling]
\label{thm:hp_low_weight_walsh}
\label{thm:fixed_phase_hp1_fisher_scaling}
    Consider the fixed-phase \myqc-1 circuit with
    \begin{equation}
        \Lambda_1=\{1,3,5,\ldots\},
        \qquad
        \Lambda_2=\{2,4,6,\ldots\},
    \end{equation}
    and
    \begin{equation}
        \theta_{ij}=\frac{2\pi}{2^{|i-j|}},
        \qquad
        i\in\Lambda_1,\quad j\in\Lambda_2.
    \end{equation}

    \textup{(a)} Its zero-flip Walsh weight is
    \begin{equation}
        |B_0(x)|^2
        =
        \prod_{j\in\Lambda_2}
        \cos^2\left(
        \pi\sum_{i\in\Lambda_1}\frac{x_i}{2^{|i-j|}}
        \right).
    \end{equation}
    ...
    \begin{equation}
        ...
        \frac{|W_r(x)|^2}{R2^n},
    \end{equation}
    and define the signed remainder
    \begin{equation}
        \Pr\nolimits_{\mathrm{bg}}(x\mid r)
        :=
    ...
    \begin{equation}
        \fii_{\mathrm{HP}}(r,n)
        =
        \bigl(1+o(1)\bigr)A_n^{\mathrm{fp}}(r)\fii_W(r,n),
    \end{equation}
    where
    \begin{align}
        \fii_W(r,n)
    ...
    Consequently, if
    \begin{equation}
        \fii_W(r,n)=\exp[k_W n+o(n)],
        \qquad
        A_n^{\mathrm{fp}}(r)=\exp[-\gamma_{\mathrm{fp}} n+o(n)],
    \end{equation}
    then
    \begin{equation}
        \fii_{\mathrm{HP}}(r,n)
        =
        \exp[(k_W-\gamma_{\mathrm{fp}})n+o(n)].
    \end{equation}
    Here $\gamma_{\mathrm{fp}}$ is the DFI-weighted effective attenuation
    exponent of $A_n^{\mathrm{fp}}(r)$, not the pointwise attenuation rate of a
    single output string.
\end{theorem}

\begin{proof}
    For the fixed-phase \myqc-1 skeleton, the local phase sum in
    Lemma~\ref{lem:hp_walsh_probability} becomes
    \begin{equation}
        \varphi_j(x)
        =
        \begin{cases}
            ...
            & j\in\Lambda_1,\\[1mm]
            \displaystyle
            2\pi\sum_{i\in\Lambda_1}\frac{x_i}{2^{|i-j|}},
            & j\in\Lambda_2.
        \end{cases}
    \end{equation}
    Therefore
    \begin{equation}
        ...
        =
        \prod_{j=1}^{n}
        \cos^2\left(\frac{\varphi_j(x)}{2}\right)
        =
        \prod_{j\in\Lambda_2}
        \cos^2\left(
        \pi\sum_{i\in\Lambda_1}\frac{x_i}{2^{|i-j|}}
        \right).
    \end{equation}
    This proves part (a). Equivalently, the pointwise attenuation rate is
    \begin{equation}
        \gamma_n(x)
        :=
        -\frac{1}{n}\ln|B_0(x)|^2
        ...
        \right),
    \end{equation}
    so $|B_0(x)|^2=\exp[-n\gamma_n(x)]$ exactly. For part (b), apply
    Lemma~\ref{lem:dfi_stability_remainder} with
    \begin{equation}
        a_n(x)=|B_0(x)|^2,
        \qquad
        \Pr\nolimits_{\mathrm{ref}}=\Pr\nolimits_W.
    \end{equation}
    The result is the displayed conditional DFI scaling, and the exponent
    formula follows from the exponent-transfer part of the lemma.
\end{proof}
