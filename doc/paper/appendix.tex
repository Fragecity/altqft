...
For the coherent law in Eq.~\eqref{eq:hp1_lag_correlation_probability},
\begin{equation}
    ...
    \right].
    \label{eq:hp1_c_dephasing_numerator_scale}
\end{equation}
\end{lemma}
...
Eq.~\eqref{eq:hp1_c_dephasing_numerator_scale}.
...
modes is enhanced by a factor of order \(2^a\), up to subexponential
corrections.  The statistical input used below is the corresponding cross-period
version of this observation: two periods can share only the trailing-zero
structure common to both of them.  We summarize this by the scaling heuristic
\begin{equation}
    \mathcal E_C(s,t)-1
    =
    \frac{N}{st}\,
...
common dyadic depth, while the polynomial factor absorbs finite-register
boundary effects and the soft width of the near-resonance condition
\(|\Gamma_C(v)|\approx1\).

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{hp1_c_twisted_dyadic_resonance.pdf}
    ...
    by exact CUDA state-vector evaluation at \(n=20\) for the fixed-phase
    \(\myqc-1\) kernel, using the pairs \(s=2^a u\), \(t=2^a v\) with
    \((u,v)=(3,5),(3,7),(5,7)\) and \(a=0,\ldots,6\).  The left panel plots
    \((\mathcal E_C(s,t)-1)st/N\), after removing the base \(N/(st)\) overlap
    scale.  The right panel divides the same quantity by \(2^a\); the result
    remains order one up to finite-size fluctuations, supporting the shared
    \(2\)-adic depth as the leading statistical enhancement.}
\end{figure}
...
\end{equation}
Then \(\nu_2(r)=n/2\), and the first term in
Eq.~\eqref{eq:hp1_c_twisted_numerator_heuristic} dominates:
\begin{equation}
    S_r
    \sim
    \operatorname{poly}(n)\frac{2^{n/2}}{2^n}
    =
    \operatorname{poly}(n)\,2^{-n/2}.
...
\begin{equation}
    \log S_r
    =
    -\frac{\log 2}{2}n+O(\log n).
    \label{eq:hp1_c_twisted_square_root_log_numerator}
\end{equation}
Thus the unweighted adjacent numerator can decay exponentially even though the
discrete Fisher information grows exponentially.

Indeed, the discrete Fisher information is
\begin{equation}
    \fii(r,n)
    =
...
\end{equation}
This explains why the log-DFI can be approximately linear in \(n\), while the
unweighted numerator \(S_r\) is exponentially small.
\end{proof}
