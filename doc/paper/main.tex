...
\begin{theorem}[Ln-linear DFI Growth]
...
\end{equation}
for constants \(k\) and \(b\) over the asymptotic scan window.
\end{theorem}

For two adjacent \myqc-1 output distributions \(\hpdist{r}\) and \(\hpdist{r+1}\), define the Chernoff information as
\begin{equation}
\begin{aligned}
    \mathcal C(\hpdist{r},\hpdist{r+1})
    &:=
    -\ln \min_{0\leq \alpha\leq 1}
    \sum_x \hpdist{r}(x)^\alpha \\
    &\qquad\qquad\qquad\qquad\times
    \hpdist{r+1}(x)^{1-\alpha}.
\end{aligned}
\end{equation}

\begin{theorem}[Local Chernoff bound of the \myqc-1 circuit]
