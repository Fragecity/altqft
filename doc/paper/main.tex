...
There has been significant progress in reducing the circuit depth required. These efforts have enabled record experimental QFT fidelities \cite{aumann2026demonstrating} through techniques including (i) approximate or truncated QFT constructions, which omit small-angle controlled rotations \cite{Coppersmith2002, park2023reducing} with \(\mathcal{O}(n\log n)\) depth, (ii) dynamic mid-circuit measurements, which reduce the resource scaling of QFT followed immediately by measurement from \(\mathcal{O}(n^2)\) two-qubit gates to \(\mathcal{O}(n)\) mid-circuit measurements without connectivity constraints \cite{baumer2024quantum}, and (iii) quantum circuit cutting and cluster-simulation methods, which trade coherent quantum depth for additional sampling and classical post-processing overhead~\cite{tang2021cutqc, Peng2020}. 
...
\begin{lemma}[Coset shifts as removable phases]
\label{lem:coset_phase_factorization}
    Let \(G=\mathbb{Z}_{2^n}\), encoded by \(n\) computational-basis bits. Suppose a unitary \(U\) has matrix elements of the form
    \begin{equation}
        \bra{x}U\ket{y}
        =
        \frac{1}{\sqrt{2^n}}
        \exp\left[\ii\sum_{i,j=1}^{n} A_{ij}x_i y_j\right],
        \qquad A_{ij}\in\mathbb{R}.
    \end{equation}
    Then \(U\) is shift-invariant for HSP sampling over \(\mathbb{Z}_{2^n}\).
\end{lemma}

For the main circuit used in the numerical sections, we take the fixed-phase \myqc-1 skeleton
\(\Lambda_1=\{1,3,5,\ldots\}\) and \(\Lambda_2=\{2,4,6,\ldots\}\), with
\(\theta_{ij}=2\pi/2^{|i-j|}\) on each controlled-phase gate between \(i\in\Lambda_1\) and \(j\in\Lambda_2\).

\begin{theorem}[Shift invariance of fixed-phase \myqc-1]
\label{thm:fixed_phase_hp1_sinv}
    The fixed-phase \myqc-1 circuit is shift-invariant for HSP sampling over \(\mathbb{Z}_{2^{n}}\).
\end{theorem}
The proof verifies that the fixed-phase \myqc-1 matrix elements have the bilinear phase form required by Lemma~\ref{lem:coset_phase_factorization} (Appendix~\ref{app:HPk are sinv}).
...
worst-case DFI: $\fii_{\min}(n):\min_{1 \leq r \leq \lfloor 2^{n/2} \rfloor} \fii(r,n)$.
Fig.~\ref{fig:fi_scaling} illustrates the minimum DFI against the qubit number $n$. (We bound the period to $1 \leq r \leq \lfloor 2^{n/2} \rfloor$ to satisfy the condition $r \ll 2^n$.)
...
The proposed \myqc~circuit family preserves an exponential growth of DFI, based on the numerical evidence. As detailed in Appendix~\ref{app:fi_log_linear_growth}, numerical data for both the \myqc{} circuits and the QFT benchmark fit the log-linear model $\ln \fii_{\min}(n) = kn + b$, with a coefficient of determination \(R^2 \geq 0.913\).
...
\begin{figure}[!t]
    ...
    \textbf{Numerical simulation shows successful Shor factoring via \myqc-$L$ and neural net post-processing.} There is a depth-7 \myqc-1 circuit with only 11 Hadamards and 30 controlled-phase gates on $11$ qubits.
    (a) Training curves for 9-, 10-, and 11-qubit instances show rapid improvement in generator recovery accuracy.
    (b) Shor-type factorization of numbers $\leq 395$ is demonstrated. Colour indicates the minimum number \(k\) of HP-\(1\)-based guesses needed for successful factorization. Most instances are solved with \(k=1\), while only a few require \(k=2\) or \(k=3\).
}
\end{figure}
...
The neural net trained successfully. As shown in Fig.~\ref{fig:recovery_pipeline}(a), the neural network achieves rapid convergence when trained on 9-, 10-, and 11-qubit instances. Both the top-1 (correct period ranked first) and top-3 accuracies increase swiftly within a few epochs.

The combined HP-1 and neural net protocol recovers the subgroup generator in the tested instances. As illustrated in Fig.~\ref{fig:recovery_pipeline}(b), each composite integer is colored by the minimum number of guesses $k$. Most instances are solved on the first attempt ($k=1$) and rest within k=3. 
