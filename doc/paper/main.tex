...
\newcommand{\hpdist}[1]{\mathrm{Pr}_{#1}}
...
\begin{equation}
\label{eq:DFI}
    \fii(r, n) = \sum_{x = 0}^{2^n -1} \frac{\left(\mathrm{Pr}(x\mid r+1) - \mathrm{Pr}(x\mid r)\right)^2}{\mathrm{Pr}(x\mid r)},
\end{equation}
where $x$ represents measurement outcomes, $r$ is the hidden subgroup generator, and $\mathrm{Pr}(x\mid r)$ is the probability of measuring $x$ when the subgroup generator is $r$.
...
worst-case DFI: $\fii_{\min}(n):=\min_{2 \leq r \leq r_{\max}(n)} \fii(r,n)$, with \(r_{\max}(n)^2\ll 2^n\) in the asymptotic statement.
Fig.~\ref{fig:fi_scaling} illustrates the minimum DFI against the qubit number $n$. (For the finite-size numerical scan, we use the square-root boundary proxy \(r_{\max}(n)=\lfloor 2^{n/2}\rfloor\), matching the period scale used in the recovery experiments.)

\begin{theorem}[Restricted-window ln-linear DFI bound]
...
The proposed \myqc~circuit family preserves an exponential growth of DFI, based on the numerical evidence. As detailed in Appendix~\ref{app:fi_ln_linear_growth}, numerical data for both the \myqc{} circuits and the QFT benchmark fit straight-line trends on the natural-log scale, with a coefficient of determination \(R^2 \geq 0.913\).
...
The outcomes are fed into a shared feature map \(\phi\), which maps each bitstring \(b_i\) to a feature vector within 2 linear layers and $16n$ feature dimension. The feature vectors are then summed to form an aggregated, permutation-invariant representation $\sum_i\phi(b_i)$. This representation conditions a parameter-shared autoregressive decoder for the binary expansion of the generator. At each decoding step the network emits 16 logits for the next four-bit block and retains the four highest-scoring prefixes in a beam search \cite{sutskever2014sequence, wiseman2016sequence}. Thus the decoder uses at most $\lceil n/4\rceil$ steps and has no output layer indexed by all candidate generators. We use polynomial samples $m = 1024n^2$ to train the neural net. The network size and beam-decoding cost are polynomial in $n$, and the resulting ranked list is passed to classical verification.
...
Fig.~\ref{fig:gate_noise_accuracy} reports the 18-qubit gate-level Pauli-noise sweep with the four-bit autoregressive decoder: the top-1 success rate drops from \(0.9627\) at \(10^{-3}\) to \(0.1314\) at \(10^{-1.5}\). Details are in Appendix~\ref{app:noise_robustness_details}.
