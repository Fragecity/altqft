...
\(C\)-twisted dyadic-resonance scaling of the adjacent-period numerator.  Passing
from this numerator to the DFI also requires the inverse-probability denominator
to be controlled on the outcomes that dominate the adjacent difference.  With
this numerator-to-denominator transfer, the numerical Fisher-information scan is
summarized by the following ln-linear law.

\begin{theorem}[Ln-linear minimum Fisher information]
\label{thm:hp1_first_order_fisher_transfer_maintext}
Restrict the adjacent-period scan to generators satisfying \(r^2\ll 2^n\).
The minimum adjacent-period DFI obeys the finite-window law
\begin{equation}
...
Theorem~\ref{thm:hp1_first_order_fisher_transfer_maintext} shows that, within the restricted period window, \myqc{} circuits retain exponentially growing information about the hidden subgroup generator.
...
Thus, adjacent generators are separated not only in the DFI sense, but also in a hypothesis-testing sense.
...
The proposed \myqc~circuit family preserves an exponential growth of DFI, based on the numerical evidence. As detailed in Appendix~\ref{app:fi_ln_linear_growth}, numerical data for both the \myqc{} circuits and the QFT benchmark fit the ln-linear model $\ln \fii_{\min}(n) = kn + b$, with a coefficient of determination \(R^2 \geq 0.913\).
