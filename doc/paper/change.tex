...
The proof of this theorem relies on a statistical regularity of the amplitudes: the adjacent probability change \(\text{Pr}(x\mid r+1)-\text{Pr}(x\mid r)\) is weighted mainly by outcomes whose probabilities remain polynomially close to the uniform background.
Theorem~\ref{thm:hp1_first_order_fisher_transfer_maintext} shows that, within the restricted period window, \myqc-1 circuits retain exponentially growing information about the hidden subgroup generator.
