...
worst-case DFI: $\fii_{\min}(n):\min_{2 \leq r \leq n^2} \fii(r,n)$.
Fig.~\ref{fig:fi_scaling} illustrates the minimum DFI against the qubit number $n$. (We bound the period by $r=\mathcal{O}(n^2)$ to satisfy the condition $r \ll 2^n$.)
...
The proposed \myqc~circuit family preserves an exponential growth trend of DFI over this polynomial period window. Appendix~\ref{app:fi_log_linear_growth} reports the corresponding log-linear regression coefficients.
