...
\(\theta_{ij}=\pi/2^{|i-j|}\) on each controlled-phase gate between \(i\in\Lambda_1\) and \(j\in\Lambda_2\).
...
\begin{equation}
\label{eq:DFI}
    \fii(r, n) = \sum_{x = 0}^{2^n -1} \frac{\left(\Pr(x\mid r+1) - \Pr(x\mid r)\right)^2}{\Pr(x\mid r)},
\end{equation}
where $x$ represents measurement outcomes, $r$ is the hidden subgroup generator, and $\Pr(x\mid r)$ is the probability of measuring $x$ when the subgroup generator is $r$. The sample complexity for estimating a discrete parameter is formally governed by the DFI (Eq.\eqref{eq:DFI}) via the Hammersley-Chapman-Robbins bound~\cite{chapman1951minimum, hammersley1950estimating}, 
\begin{equation}
...
For \myqc{} circuits, the DFI is evaluated numerically. Specifically, for a fixed $r$, the circuit maps the periodic input components $\{\ket{qr}\}_q$ to a computational-basis distribution over outcomes $\ket{b}$, with the exact probabilities given by $\Pr(\ket{b}) = \left|\sum_q \langle b|U|qr\rangle\right|^2$.
...
The proposed \myqc~circuit family preserves an exponential growth trend of DFI over this square-root period window. Appendix~\ref{app:fi_log_linear_growth} reports the corresponding log-linear regression coefficients.
...
\begin{figure}[!t]
    ...
    (b) Shor-type factorization on 18-qubit semiprime instances with \(N=225001,\ldots,254999\). Colour indicates the minimum number \(k\) of HP-\(1\)-based guesses needed for successful factorization, with grey entries corresponding to cases without a suitable base \(a\) under the period bound.
}
\end{figure}
