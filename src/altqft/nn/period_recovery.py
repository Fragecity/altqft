...
def _sample_columns_from_distribution(
    ...
    columns = np.repeat(
        np.arange(distribution.size, dtype=np.int64),
        counts.astype(np.int64, copy=False),
    )
    if columns.size != sample_count:
        raise RuntimeError("sampled column count does not match requested sample_count")
    return columns
